...
async def send_verification_email(email: str, name: str, verification_token: str):
    """Send verification email using Resend - MUST work reliably"""
    import resend
    ...
    # Always use production URL for verify links
    verify_url = f"https://blendlink.net/verify-email?token={verification_token}&email={email}"
...
<body style="margin: 0; padding: 0; background-color: #f0f2f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
    <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        ...
        <div style="background: #ffffff; border-radius: 16px; padding: 40px 32px; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
            ...
            <p style="color: #374151; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
                Thanks for creating your BlendLink account! Please verify your email address to activate your account and unlock all features.
            </p>
            <div style="background: #f8fafc; border-radius: 12px; padding: 16px; margin-bottom: 32px; border: 1px solid #e5e7eb;">
                <table style="width: 100%;"><tr><td style="color: #6b7280; font-size: 13px; padding: 4px 0;">Name</td><td style="color: #111827; font-size: 14px; font-weight: 600; text-align: right;">{name}</td></tr><tr><td style="color: #6b7280; font-size: 13px; padding: 4px 0;">Email</td><td style="color: #111827; font-size: 14px; font-weight: 600; text-align: right;">{email}</td></tr></table>
            </div>
            <div style="text-align: center; margin: 32px 0;">
                <a href="{verify_url}" style="display: inline-block; background: linear-gradient(135deg, #00F0FF, #BD00FF); color: white; text-decoration: none; padding: 16px 48px; border-radius: 50px; font-weight: 700; font-size: 16px; box-shadow: 0 4px 16px rgba(0, 240, 255, 0.3);">
                    Verify My Email
                </a>
            </div>
    ...
    try:
        ...
        # Use synchronous call wrapped in to_thread for reliability
        result = await asyncio.to_thread(resend.Emails.send, params)
        logger.info(f"Verification email sent to {email} - ID: {result}")
        return True
    except Exception as e:
        logger.error(f"FAILED to send verification email to {email}: {type(e).__name__}: {e}")
        # Try one more time synchronously as fallback
        try:
            result = resend.Emails.send(params)
            logger.info(f"Verification email sent (retry) to {email} - ID: {result}")
            return True
        except Exception as e2:
            logger.error(f"RETRY ALSO FAILED for {email}: {e2}")
            return False
