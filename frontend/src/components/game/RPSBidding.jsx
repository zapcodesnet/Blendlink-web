...
export const RPSBidding = ({
  ...
  const generateBotChoice = useCallback(() => {
    const strategies = {
      easy: () => {
        // Random choice and low bid
        const choice = RPS_CHOICES[Math.floor(Math.random() * 3)].id;
        const bid = Math.min(opponentMoney, MIN_BID * (Math.floor(Math.random() * 2) + 1));
        return { choice, bid };
      },
      medium: () => {
        // Random choice, variable bid
        const choice = RPS_CHOICES[Math.floor(Math.random() * 3)].id;
        const maxAffordable = Math.min(opponentMoney, MAX_BID);
        const bid = BID_OPTIONS.filter(b => b <= maxAffordable)[
          Math.floor(Math.random() * BID_OPTIONS.filter(b => b <= maxAffordable).length)
        ] || MIN_BID;
        return { choice, bid };
      },
      hard: () => {
        // Strategic choice based on previous rounds, higher bids
        const choice = RPS_CHOICES[Math.floor(Math.random() * 3)].id;
        const maxAffordable = Math.min(opponentMoney, MAX_BID);
        // Hard bot tends to bid higher
  ...
  const handleSubmit = useCallback((timeout = false) => {
    ...
    if (rpsResult === 'player1') {
      winner = 'player';
      playerWinnings = playerBid + botMove.bid; // Winner takes pot
      ...
      if (playerBid > botMove.bid) {
        winner = 'player';
        playerWinnings = playerBid + botMove.bid;
        setRoundResult('player1');
      } else if (botMove.bid > playerBid) {
        winner = 'opponent';
        setRoundResult('player2');
      } else {
        // Same bid - true tie, no pot change
        winner = 'tie';
        setRoundResult('tie');
      }
    }
    ...
    <div className="relative bg-gray-800/50 rounded-2xl p-4 sm:p-6 border border-gray-700/50" data-testid="rps-bidding">
      ...
      {gamePhase === 'choosing' && !playerBankrupt && (
        <>
          ...
          {/* Bid Selection */}
          <div className="mb-6">
            <p className="text-sm text-gray-400 mb-3 text-center">Set your bid (min $1M):</p>
